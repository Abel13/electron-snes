...
const drawTriangle = (
  ...
  context.drawImage(image, sourceLeft, sourceTop, sourceWidth, sourceHeight, 0, 0, sourceWidth, sourceHeight);
  context.restore();
};

const roundedPath = (context: CanvasRenderingContext2D, points: readonly PerspectiveLabelPoint[]): void => {
  const adjusted = points.map((point, index) => {
    const previous = points[(index + points.length - 1) % points.length]!;
    const next = points[(index + 1) % points.length]!;
    const radius = Math.min(point.radius, Math.hypot(point.x - previous.x, point.y - previous.y) / 2, Math.hypot(next.x - point.x, next.y - point.y) / 2);
    ...
    return {
      point,
      before: { x: point.x + ((previous.x - point.x) * radius) / beforeLength, y: point.y + ((previous.y - point.y) * radius) / beforeLength },
      after: { x: point.x + ((next.x - point.x) * radius) / afterLength, y: point.y + ((next.y - point.y) * radius) / afterLength },
    };
  });
...
export const PerspectiveLabelCanvas = (props: {
  ...
  useEffect(() => {
    ...
    const redraw = (): void => {
      ...
      const points = [props.map.topLeft, props.map.topRight, props.map.bottomRight, props.map.bottomLeft].map((point) => ({ x: (point.x / 100) * width, y: (point.y / 100) * height, radius: point.radius })) as [PerspectiveLabelPoint, PerspectiveLabelPoint, PerspectiveLabelPoint, PerspectiveLabelPoint];
      context.save();
      ...
      for (let row = 0; row < steps; row += 1)
        for (let column = 0; column < steps; column += 1) {
          ...
          const source = (u: number, v: number): Point => ({ x: u * image.naturalWidth, y: v * image.naturalHeight });
          const a = source(u0, v0), b = source(u1, v0), c = source(u1, v1), d = source(u0, v1);
          const A = bilinear(u0, v0), B = bilinear(u1, v0), C = bilinear(u1, v1), D = bilinear(u0, v1);
