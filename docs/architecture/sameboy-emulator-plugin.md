...
The bridge exposes ROM loading, normalized Game Boy buttons, a 160x144 RGBA video frame,
audio configuration, and SameBoy's battery RAM/RTC buffers. It does not receive filesystem
paths, Electron APIs, physical-device APIs, or user data directories.
...
The platform ships no commercial ROMs, proprietary boot ROMs, saves, or save states. User
ROMs are introduced only through an explicit host boundary. User cartridge saves are
loaded and persisted locally as described in `cartridge-saves.md`.
